...
import { MetronomeVisualMode } from '../types';
...
export const Metronome: React.FC<MetronomeProps> = ({ 
  ...
  // Helper Flags
  const isFlashEnabled = visualMode === MetronomeVisualMode.FLASH || visualMode === MetronomeVisualMode.BOTH;
  const isDotsEnabled = visualMode === MetronomeVisualMode.DOTS || visualMode === MetronomeVisualMode.BOTH;

  // Full Screen Flash Logic
  const getFlashStyle = () => {
    if (!isFlashEnabled || activeBeat === -1) return '';
    
    // Strong beat (0) = Gold flash, Weak beat = White/Zinc flash
    if (activeBeat === 0) {
      return 'bg-brass-500/30 shadow-[inset_0_0_100px_rgba(234,179,8,0.5)]';
    }
    return 'bg-zinc-500/10 shadow-[inset_0_0_50px_rgba(255,255,255,0.1)]';
  };

  return (
    <>
      {/* Full Screen Flash Overlay (Fixed) */}
      {isFlashEnabled && activeBeat !== -1 && (
         <div 
           className={`fixed inset-0 z-0 pointer-events-none transition-all duration-75 ${getFlashStyle()}`} 
         />
      )}

      <div className={`w-full max-w-[340px] mx-auto ${className} relative z-10`}>
          ...
          <div className="flex items-center justify-between mb-1.5 px-1">
            ...
            <div className="flex gap-2 items-center h-3">
              {isDotsEnabled ? (
                ...
                // FLASH ONLY MODE INDICATOR (Small icon to show metronome is active)
                <div className="flex items-center gap-1.5 opacity-70">
                   <Zap size={12} className={isPlaying ? "text-brass-400" : "text-zinc-600"} />
                   <span className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest">Screen Flash</span>
                </div>
              )}
            </div>
